...
- [x] 2.2 系統判定フォールバックと違法種別の分類（TDD）
  ...
  - 受け入れ条件: 完全重複起因の**管理対象カテゴリー**保有が FIX ではなく DUP_ONLY になることを
    含めること。task 2.1 時点では DUP_ONLY 未実装のため、以下が破壊的 FIX へ落ちる
    （**本タスク完了前に CatRacerCleanupShell（task 4.x）を配線しないこと**。
    task 2.1 round-2独立レビュー MAJOR-2、round-3独立レビュー MINOR-1/MINOR-2で訂正・追記）:
    - 【正確な最小再現、round-3で訂正】単純な `{C1, C1}` のみの重複は現状 MANUAL に落ちるため
      破壊的ではない。実際に破壊的FIXになるのは**重複が正系統と反対側にある**ケースで、例えば
      `{C1#1, C1#2, CM1#3}` ＋ Masters出走 → `FIX keep=CM1, cancel=[C1#1,C1#2], grant=C2`
      となり、C1 が2行とも終了され Requirement 4.5 により再付与不能（実質降格が復旧不能）
    - 【round-3で追記、未記録だった別ケース】重複が対応外ペアと併存する混在ケースでは、重複行が
      cancel 対象にも grant 抑止判定にも含まれず、「何も変更しない no-op FIX」（例:
      `{CM1,C2,C2}` ＋ Masters出走 → `cancel=[], grant=null`）や「是正後も違法が残るFIX」
      （例: `{CM1,C4,C2,C2}` ＋ Masters出走 → cancel後の集合が依然`{CM1,C2,C2}`で
      DUPLICATE_CATEGORY のまま）が発生し、Requirement 7.1の「是正後 違法ゼロ」を達成できない。
      DUP_ONLY設計時に、重複と他の違法状態が併存する混在ケースの扱い（DUP_ONLYへ倒すか、
      FIXしつつ重複行も終了対象に含めるか）を決定すること
...
- **タスク1.1（2026-08-29実装完了、独立レビュー4巡）**: フィクスチャに「シナリオが存在するだけで
  判定結果を左右しない（non load-bearing）」行を追加してしまう失敗パターンを3巡連続で踏んだ
  （デバッグサブエージェントで根本原因分析・修正、4巡目で承認）。原因は各修正が指摘された項目
  単体だけを直して、他の既に load-bearing だったシナリオへの副作用や、同種の別ケース（DNS除外は
  直したがDNF行の日付関係までは見ていなかった等）を確認していなかったこと。**教訓**: 「フィルタ
  Xを1つだけ外すと結論が変わるか」を、関係する全フィルタについて1つの統合表（出走チェーン全体・
  category_racersの複合条件行列）から機械的に再導出して確認する手順を、今後同種のフィクスチャ
  設計タスク（task 2.x以降の統合テスト拡張、他specでの同パターン）でも最初から踏むこと。
  最終承認時の統合表・9種フィルタの独立検証結果はagreement-log.md参照。
