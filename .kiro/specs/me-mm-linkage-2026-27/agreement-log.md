...
## ME1保有者の再定義と連動の非降格原則（2026-09-25）

### 発端

本番（2026-09-20〜22）で、エントリー時の対応ペア補完（entry-auto-category-2026-27）がCM1種目への
エントリーを契機に、過去にC1を保有していただけで現在はエリート側を保有していない選手3名へC1を
新規付与した（category_racers id 268502 CXK-156-0112 / 268546 CCS-167-0033 / 268556
KNS-000-0601）。本番サーバーログ（2026-09-24取得）の`entry_auto_category.log`とDBダンプで確認済み。

### 根本原因

1. **仕組み上の原因**: `CategoryLineageLinker::resolveLinkedTarget()`がCM1を起点とする連動先を、
   `isFormerElite1()`（削除されていないC1行が過去に1件でもあればtrue。取消済みかどうか・いつの
   記録かは問わない）がtrueならC2ではなくC1へ引き上げていた。エントリー補完・リアルタイム昇格
   連動の両方がこのメソッドを共有する。
2. **仕様上の原因**: 本spec第2版Requirement 4.3・5.1・5.3が、AJOCC規則「MM1からME1への移行は、
   元がME1の場合に限る」を「過去にME1だった選手はMM1到達時にME1へ連動させる」と読んでいた。
   同規則は移行を**認める範囲の上限**を定めたものであり、自動付与の根拠ではない。
3. **プロセス上の原因**: catracer-cleanup-2026-27 research.mdは「元ME1でもC1は自動付与しない」と
   正しく決定していたが、その決定が対応表の正本である本specにフィードバックされず、spec間の
   食い違いが残った。entry-auto-category-2026-27は「既存ロジックを無変更で流用する」ことを理由に、
   特例をエントリーの文脈で使ってよいかを再検討しなかった。
4. **テストでの固定**: `testMastersCm2ToCm1PropagatesEliteToC1ForFormerMe1`などが、元ME1の選手が
   CM1へ昇格するとエリート側がC3→C1へ2段階上がる挙動を「正しい」として固定していた（リアルタイム
   昇格側では本番の発生0件。CM1への昇格連動14件はすべてC2）。

### 決定事項（人間 2026-09-25）

| # | 決定内容 |
|---|---|
| 1 | 「元ME1」を廃し「ME1保有者」とする。判定基準日（エントリーなら大会日、昇格ならレース日）時点でエリート側にC1を有効保有している状態を指し、過去の保有歴は用いない |
| 2 | 対応表: C2⇔CM1・C3⇔CM2・C4⇔CM3は双方向、C1→CM1は一方向。いずれも系統連動で自動付与できる。CM1の相手は常にC2 |
| 3 | CMしか持たない選手がCM1に関わる場合（エントリー・昇格）に自動付与されるのはC2。過去のC1保有歴は影響しない |
| 4 | ME1保有者がCMの種目へエントリーしても補完は発動しない（C1を元々保有しているため）。通知はログのみ |
| 5 | **連動の非降格原則**: 対応外ペアは過去の参加履歴により選手によっては存在しうる。昇格による連動が相手系統の現保有カテゴリーを降格させることはない（相手系統が連動先と同位以上を保有していれば何もしない）。ME1保有者のC1はこの原則で保護される。C1以外の対応外ペア（例: C2＋CM3でCM3→CM2昇格時にC2を取り消してC3を付与、C4＋CM1でC4→C3昇格時にCM1を取り消してCM2を付与）で同じ降格が起きる既存不具合も本改訂のスコープに含める |
| 6 | 手動操作でC1を付与したときの警告（Requirement 5.4・6.4）は存続させる。判定は「付与直前にME1保有者でなかったか」に変更する（過去にME1歴がある選手でも、昇格以外でCM1保有者へC1を手動付与すれば警告が出る。保存は完了する） |
| 7 | 誤付与3件のデータ是正は本改訂のスコープ外とし、後から別途実施する |
| 8 | Spec本文では「元ME1」という用語を使わず「ME1保有者」に統一する |

### 改訂内容

- requirements.md 第3版: Requirement 1.2・4.3・4.7・5（全面）・6.4を改訂、4.9を新設
- entry-auto-category-2026-27 requirements.md 第6版（Requirement 1.5新設・2.2改訂、同spec決定事項#15）
- design.md・tasks.mdは、requirements承認後に改訂する
- 承認状態: spec.jsonのrequirements/design/tasksのapprovedをfalseに巻き戻した
- **2026-09-25 requirements第3版を人間承認**（entry-auto-category-2026-27 第6版と同時）
- **2026-09-25 design第3版を人間承認**（entry-auto-category-2026-27 design第6版と同時）。tasks第3版（S1〜S7、entry側E1）を作成
- 2026-09-25 design第3版（改訂案）を作成。設計判断: (1) `isFormerElite1()`・`resolveLinkedTarget()`は置き換えでなく削除（過去の保有歴を見る判定や「特例があるように読める解決メソッド」を残すと誤用を招くため。本番コードの呼び出し元は自spec内のみと確認済み）、(2) 非降格の比較は相手系統の有効行すべてのうち最上位で行い、引き上げる場合もcancelするのは最上位の1行だけ（既存不整合行には触れない方針に合わせる）、(3) 非降格で見送った場合は新しい結果`NO_PROPAGATION_HIGHER_HELD`を返し、昇格処理側でINFOログを残す、(4) エントリー補完のME1保有者は新しい結果種別を作らず`noSupplementAlreadyValid('C1')`を返す

### 実装・検証結果（2026-09-25）

- 第3版タスクS1〜S6・entry-auto-category E1を実装（cyclox2web `fix/me1-holder-linkage-2026-27`）。
  関連14スイート304テストGREEN、ミューテーション7種すべて検知（test-results.md）
- tester: PASS-WITH-NOTES（指摘3件は対応・受容済み）。reviewer: APPROVED（ブロッキングなし）
- **人間判断（2026-09-25）**: reviewer MODERATE-1（手動C1警告の判定日を保存行のapply_dateとしているため、
  終了済みC1行の再有効化や過去日付でのC1付与で警告が出ない）とMINOR-1（apply_date前倒し編集での誤警告）は
  後続タスクに先送りする。本番事象（自動付与によるC1誤付与）の再発防止は達成済みで、残る論点は手動操作時の
  注意表示の精度であり、データは壊れないため
