...
- **Findings**:
  ...
  - `CM1+2+3` レースは別経路（L613-641: 優勝→CM1、表彰台→CM2）で判定される。
    **【2026-09-24訂正】この記述は誤り。実際には`__rule0123`（境界10/20/40）を使用して
    おり枠数の概念もある。この誤読がdesign.md L298の「本改正の対象外」判断の根拠に
    なっていたが、実際にはAJOCC公式ページの規定（MM1+2+3の2-3位昇格はMM3→MM2の出走
    人数基準を適用）と食い違う別バグが存在し、season-rules-2026-27フォローアップ
    （agreement-log.md「2026-09-24（続き）CM1+2+3経路の派生課題調査・修正」節）で
    26-27シーズン以降のみ`__rule012333`へ切り替える形で別途修正した。**
