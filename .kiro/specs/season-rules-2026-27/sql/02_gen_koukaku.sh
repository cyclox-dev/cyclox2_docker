#!/bin/bash
# =====================================================================
# downlist 出力 + 降格SQL(koukaku)生成 2026-27版（再利用可能）
...
#        ただしC2+CM1・C1+CM1・C3+CM2の組み合わせは連動降格として正常系のため
#        除外対象ではない（01_build_demote_set.sql末尾の異常判定を参照）。
#
# 2025-26版（rider-demotion-2025-26/sql/02_gen_koukaku.sh）からの変更点は
# 年次パラメータ（APPLY/CANCEL/NOTE）のみ。demote_allのスキーマ・生成ロジックは
# 互換のため本スクリプト自体にペア認識ロジックは持たせない（判定は01に集約）。
# =====================================================================
set -euo pipefail
...
# 降格先マッピング（me-mm-linkage-2026-27 CategoryLineageMapを出典とする複製。
# C1->C2, C2->C3, C3->C4, CM1->CM2, CM2->CM3, CL1->CL2。対応表変更時は要追随）
...
emit_downlist CL1 we1_downlist.txt "ord_rank IS NULL, ord_rank, racer_code"
