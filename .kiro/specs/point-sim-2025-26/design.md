...
- **DB変更**: 2025-26の `calc_rule=11` シリーズ（35件）を `calc_rule=13` へ更新（SQL、`season_id=16`限定）。他シーズンのJCX_245利用シリーズは不変。
...
- **シリーズランキング**: `PointSeriesController::calcup`（`ranking_<id>.csv` 生成）→ `download_point_ranking_csv`。対象は 2025-26 JCF/JCXシリーズ（35件、calc_rule=13へ変更後）。
