...
2. 残留ライン（ME順位・MM点数・WE1基準）を**AJOCC公式で確認**して置換。
   > **【2026-08-29申し送り・2026-27シーズン末（2027年3月）作業時に必読】**
   > AJOCC公式ルール（<https://www.cyclocross.jp/about/rules-regulations.html>）を2026-08-29に
   > 確認したところ、2026-27シーズンから残留ラインが変更される（`season-rules-2026-27`spec
   > design.md参照、requirements/design承認済みだが本番未実装）：
   > - ME2（C2）: 260位 → **240位**
   > - ME3（C3）: 280位 → **240位**
   > - ME1（C1）: 240位のまま変更なし
   > - MM1（CM1）: 80点のまま変更なし、MM2（CM2）: 40点のまま変更なし
   > - WE1（CL1）: 出走ベース（従来は順位基準なし） → **80位**（新規に順位基準を導入）
   > - あわせてME⇔MM対応ペア間の系統横断残留判定・連動降格判定が新たに導入される
   >   （設計済み・未実装。適用する場合は本ファイルの機械的な数値差し替えでは足りず、
   >   `season-rules-2026-27`spec のtasks.md task 2〜4を先に実装する必要がある）。
   > 上記の数値はAJOCC公式ページの記載内容の要約であり、実際の適用前に必ず公式ページで
   > 再確認すること（本申し送り作成後にルールが再改訂されている可能性があるため）。
