...
※ME1→MM1 の一方向。MM1 の相手は常に ME2 で、連動で ME1 を付与することはない。2026-09-25
me-mm-linkage-2026-27 第3版で「元ME1（過去の保有歴）」を「ME1保有者（現に C1 を保有）」に再定義）。あわせて、二重付与（対応外ペアの同時保有）を仕組みで防止し、既存の不整合
...
- **Why**: 既存の `category_racers` 構造・履歴管理をそのまま使え、改修範囲を局所化できる。
  ME1⇔MM1 の非対称制約も履歴（元ME1判定）で表現可能。（**2026-09-25 訂正**: 履歴による判定は
  C1 誤付与の原因となったため廃止。現に C1 を保有しているか（ME1保有者）で判定する）
